...
def main():
    ...
    for case in [0, 1, 2]:
        ...
        pred_latent_state = torch.cat([latent_state[:, 0:init_time_steps], pred_latent_state], dim=1)
        pred_hf_trajectory = AE.decoder(
            pred_latent_state.squeeze(0), 
            pars.repeat(init_time_steps+output_seq_len, 1)
            )
